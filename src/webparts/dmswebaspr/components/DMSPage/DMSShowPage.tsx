                ...
                try {
                    ...
                    const mapped: IFileItem[] = await Promise.all([
                        ...subFolders
                            ...
                            .map(async (f) => {
                                ...
                                const item = await sp.web
                                    .getFolderByServerRelativePath(f.ServerRelativeUrl)
                                    .listItemAllFields
                                    .select("EffectiveBasePermissions")();

                                const perms = item.EffectiveBasePermissions;

                                const canDelete =
                                    sp.web.hasPermissions(perms, PermissionKind.DeleteListItems) ||
                                    sp.web.hasPermissions(perms, PermissionKind.ManagePermissions);
                        ...
                        ...fileItems.map(async (f) => {
                            // 🔐 permission check
                            const item = await sp.web
                                .getFileByServerRelativePath(f.ServerRelativeUrl)
                                .listItemAllFields
                                .select("EffectiveBasePermissions")();
            ...
            <div className={`document-page ${isArabic ? "rtl" : "ltr"}`}
                ...
                {showModalFile && (
                    <div className="modalOverlay">
                        <div className="modalContent">
                            <div className="modelbox">
                                <h3>{isArabic ? "تحميل المستند" : "Document Upload Procedure"}</h3>
                            </div>
